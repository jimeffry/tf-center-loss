...
class FaceDetector(object):
    def __init__(self,prefix,file_in='None',min_size=24,score_threshold=[0.5,0.7,0.9],\
                batch_size=[1,256,32],epoch_load=[32,30,25],nms_thresh=[0.5,0.3,0.3],test_relu=1):
    ...
    def get_face(self,img):
        ...
        if len(bbox_clib)>0:
            ...
            w_ = bbox_clib[:,2] - bbox_clib[:,0]
            h_ = bbox_clib[:,3] - bbox_clib[:,1]
            s_ = w_ * h_
            index_ = np.argsort(s_)[::-1]
            bbox_clib = bbox_clib[index_,:]
            return bbox_clib
        else:
            return None
